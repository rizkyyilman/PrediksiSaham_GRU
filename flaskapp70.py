...
def predict(stock):
    ...
    historical_predictions = [model.predict(data[i:i+time_step].reshape((1, time_step, len(numeric_cols))), verbose=0)[0, 0] for i in range(len(data) - time_step)]
    ...
    trace_future = go.Scatter(x=future_dates, y=future_prices, mode='lines', name='Prediksi Masa Depan (6 Bulan) 70', line=dict(color='green'))
    ...
    r2 = r2_score(y_test, model.predict(X_test, verbose=0))
