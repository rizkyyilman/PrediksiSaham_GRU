...
from tensorflow.keras.optimizers import RMSprop  # Mengganti Adam dengan RMSprop
...
def build_gru_model(input_shape):
    ...
    model.compile(
        optimizer=RMSprop(learning_rate=0.01),  # Ganti optimizer Adam dengan RMSProp
...
def predict(stock):
    ...
    model_path = os.path.join(MODEL_DIR, f"{stock}_gru_model_60_100_rmsdrop.h5")
    ...
    if os.path.exists(model_path):
        ...
        model.fit(X_train, y_train, validation_split=0.4, epochs=100, batch_size=64, callbacks=[early_stop], verbose=0)
    ...
    layout = go.Layout(
        title={
            'text': f'Harga Saham {stock} - Historis dan Prediksi 6 Bulan ke Depan (Januari - Juni) 2025 with RMSDrop 60/40',
            'x': 0.5,
            'xanchor': 'center'
        },
        autosize=True,
